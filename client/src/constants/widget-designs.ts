...
export function isWidgetDesignId(value: unknown): value is WidgetDesignId {
  return typeof value === "string" && (WIDGET_DESIGN_IDS as readonly string[]).includes(value);
}

export function shiftWidgetDesign(id: WidgetDesignId, delta: number): WidgetDesignId {
  const index = WIDGET_DESIGN_IDS.indexOf(id);
  const next = (index + delta + WIDGET_DESIGN_IDS.length) % WIDGET_DESIGN_IDS.length;
