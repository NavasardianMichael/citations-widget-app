...
const TABS: {
  name: string
  href: Href
  labelKey: 'tabs.citations' | 'tabs.submit' | 'tabs.settings' | 'tabs.profile'
  icon: keyof typeof MaterialIcons.glyphMap
}[] = [
  {
    name: 'index',
    href: '/',
    labelKey: 'tabs.citations',
    icon: 'format-quote',
  },
...
export default function TabsLayout() {
  ...
  return (
    <Tabs>
      ...
      <TabList asChild>
        <View className='fixed bottom-0 left-0 right-0 z-50 flex-row items-center justify-between border-t border-outline-variant bg-surface px-3 py-2'>
          {TABS.map((tab) => (
  ...
  return (
    ...
    >
      <MaterialIcons
        name={icon}
        size={20}
        color={isFocused ? '#021a35' : '#44474d'}
      />
      <Text
        className={`font-body-md text-body-md ${isFocused ? 'text-primary' : 'text-on-surface-variant'}`}
      >
        {label}
      </Text>
    </Pressable>
