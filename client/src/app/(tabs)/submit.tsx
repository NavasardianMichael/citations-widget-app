...
export default function SubmitScreen() {
  ...
  async function handleSubmit(visibility: "private" | "pending") {
    ...
    try {
      ...
      Alert.alert(
        visibility === "private" ? "Saved" : "Submitted",
        visibility === "private" ? "Saved to your custom list." : "Submitted for review.",
        [
      ...
      Alert.alert("Error", e instanceof Error ? e.message : "Failed to submit citation");
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <ScrollView className="flex-1" contentContainerClassName="pb-28 md:pb-12">
        <View className="mx-auto w-full max-w-2xl px-margin-mobile py-8 md:px-margin-desktop md:py-12">
          <View className={`mb-12 ${isMd ? "items-start" : "items-center"}`}>
            ...
            >
              Contribute to the sanctuary. All submissions are gently reviewed by our archivists before joining the
              public collection.
            </Text>
          </View>
              ...
              <View className="flex-col gap-4 md:flex-row md:items-center md:justify-end">
                {isMd ? (
                  <View className="mr-auto flex-row items-center gap-2">
                    ...
                    <Text className="font-label-sm text-label-sm text-on-surface-variant">Submissions pending review</Text>
                  </View>
                ) : null}
                <Button
                  label="Save to My Custom List"
                  variant="secondary"
                  icon="bookmark-add"
