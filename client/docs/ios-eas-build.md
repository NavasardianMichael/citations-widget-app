...
| Home-screen widget is a white/dark empty rectangle with a gray bar | WidgetKit placeholder. TestFlight Release used to hide JS/Swift errors as `EmptyView`. After the next native iOS build the widget should show either the quote or a **red error**. Open the app once, then remove and re-add the widget. Confirm App Group `group.com.anonymous.citationswidgetapp` on both the app and `.widgets` identifiers. |
