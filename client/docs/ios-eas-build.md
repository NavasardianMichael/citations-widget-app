...
## The two platforms use different identifiers — on purpose

| | Identifier |
|---|---|
| Android (`android.package`) | `com.mnavasardian.citations` |
| iOS (`ios.bundleIdentifier`) | `com.anonymous.citationswidgetapp` |

`com.anonymous.*` is the Expo prebuild placeholder. Android was renamed off it in
September 2026 because a Play package name is permanent from the first upload and
the Play release was imminent.

**iOS was deliberately left on the placeholder.** Renaming it means a new App Store
Connect record, new provisioning, a new Google iOS OAuth client, TestFlight testers
reinstalling — and, most disruptive at the time, a new App Group, which would have
invalidated the in-flight widget-sync debugging that was running against
`group.com.anonymous.citationswidgetapp`. None of that buys anything for a Play-only
release.

### Read this before the first App Store submission

An Apple bundle identifier is permanent once an app is **released** to the App Store.
TestFlight does not lock it in. So the rename is free right up to that moment and
impossible afterwards.

**Rename iOS before you submit for App Store review.** Four values in
[`../app.json`](../app.json):

| Field | Change to |
|---|---|
| `ios.bundleIdentifier` | `com.mnavasardian.citations` |
| `ios.entitlements` → application-groups | `group.com.mnavasardian.citations` |
| `expo-widgets` plugin → `bundleIdentifier` | `com.mnavasardian.citations.widgets` |
| `expo-widgets` plugin → `groupIdentifier` | `group.com.mnavasardian.citations` |

Then: update the bundle IDs in this file, create a new iOS OAuth client in Google
Cloud Console against the new bundle ID, update `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`
locally and in EAS production env, let EAS regenerate provisioning on the next
build, and re-test the widget — the App Group change means the extension reads from
a container that starts empty.
