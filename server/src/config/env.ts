...
const envSchema = z.object({
  ...
  /**
   * Android OAuth client IDs, comma-separated — accepted as id_token audiences for mobile
   * sign-in. An Android client is bound to one signing key, so there is one per key: the
   * debug keystore (local APKs) and Play's app signing key (installs from Play).
   */
